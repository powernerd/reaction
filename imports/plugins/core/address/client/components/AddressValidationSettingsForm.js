  ...
  render() {
    ...
    return (
      <Form ref={(formRef) => { this.form = formRef; }} onSubmit={onSubmit} validator={validator} value={value}>
        <Field name="serviceName" label={i18next.t("addressValidation.serviceName")} labelFor={serviceNameInputId}>
          <Select id={serviceNameInputId} name="serviceName" options={serviceOptions} />
          <ErrorsBlock names={["serviceName"]} />
        </Field>
        <Field name="countryCodes" label={i18next.t("addressValidation.countryCodes")} labelFor={countryCodesInputId}>
